...
import type { AddSectionPayload, UpdateSectionPayload } from '../src/schemas.js';
...
describe('TestRailClient - Sections CRUD', () => {
    ...
    it('should add a section (success)', async () => {
        ...
        const result = await client.sections.addSection(1, payload);
        expect(result).toEqual(mockSection);
    });

    it('should validate projectId for addSection', async () => {
        const payload: AddSectionPayload = { name: 'New Section' };
        await expect(client.sections.addSection(0, payload)).rejects.toBeInstanceOf(TestRailValidationError);
    });
    ...
    it('should throw API error for addSection when API fails', async () => {
        ...
        const payload: AddSectionPayload = { name: 'New Section' };
        await expect(clientNoRetries.sections.addSection(1, payload)).rejects.toBeInstanceOf(TestRailApiError);
    });
    ...
    it('should update a section (success)', async () => {
        ...
        const result = await client.sections.updateSection(1, payload);
        expect(result).toEqual(mockSection);
    });

    it('should validate sectionId for updateSection', async () => {
        const payload: UpdateSectionPayload = { name: 'Updated' };
        await expect(client.sections.updateSection(-1, payload)).rejects.toBeInstanceOf(TestRailValidationError);
    });
    ...
    it('should throw API error for updateSection when API fails', async () => {
        ...
        const payload: UpdateSectionPayload = { name: 'Updated' };
        await expect(clientNoRetries.sections.updateSection(1, payload)).rejects.toBeInstanceOf(TestRailApiError);
    });
    ...
    it('should validate sectionId for deleteSection', async () => {
        await expect(client.sections.deleteSection(0)).rejects.toBeInstanceOf(TestRailValidationError);
    });

    it('should throw API error for deleteSection when API fails', async () => {
        const clientNoRetries = createClient({ maxRetries: 0 });
        mockFetch.mockResolvedValueOnce(mockErr(500, 'Internal Server Error', 'Boom'));
        await expect(clientNoRetries.sections.deleteSection(1)).rejects.toBeInstanceOf(TestRailApiError);
    });
});
