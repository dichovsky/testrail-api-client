...
```ts
if (existsSync(target) && !opts.force) {        // L67 — existsSync FOLLOWS symlinks
...
*before* the calling module runs Zod validation:
...
**Effect:** a *single* 2xx response with a JSON-valid but schema-invalid
...
This is the *exact* class of bug the `clearCache()`-on-mutation logic
...
3. **Chained with finding #1's residual TOCTOU:** even after the
   ...
   call. A DNS rebind that lands a *single* request on an internal
...
- Add an integration test that exercises the "one bad response →
  ...
  the second read makes a fresh fetch (i.e. is *not* served from
  cache).
...
A symmetric typo on the destructive flag fails *safe*: `--y` instead
...
mis-flagged; `--dryrun` errors *quietly* by executing.
...
The `--soft` row is the *second* high-severity case after `--dry-run`:
the entire `case delete-bulk --soft` workflow (per BACKLOG's lock-in
note: "`--soft` adds TestRail's `soft=1` query param — a *server-side*
...
const KNOWN_FLAGS = new Set([
    'base-url', 'email', 'api-key', 'format', 'quiet', 'help', 'version',
    'project-id', 'suite-id', 'run-id', 'case-id', 'limit', 'offset',
    'data', 'data-file', 'dry-run', 'global', 'force', 'print-path',
    'file', 'filename', 'out', 'yes', 'soft',
]);
...
that *could* contain a password (e.g. a future `user add` payload —
...
*credential* — the secret that authorises every API call — has no
alternative path.
...
3. **5xx retry amplification** — the request path retries on 5xx
   ...
   them, but the *peak* memory is still ~500 MB plus the previous
...
4. **Error path amplification on the same call** — even when the
   ...
   *and* delays the throw long enough that the rate-limiter
...
5. **Cache amplification (chains with finding #9)** — if the
   ...
   holds the oversized object resident *and* serves it back on
...
2. Read bodies through a helper that consumes
   ...
   ```ts
   async function readBodyCapped(response: Response, cap: number,
                                  controller: AbortController): Promise<Uint8Array> {
...
| `update_*`                | Repeated; latest-write-wins, *usually* harmless                            |
...
3. **(Optional) Surface a `retryablePost: boolean` opt-in on
   `TestRailConfig`** for callers who *want* the old behaviour
...
*same object identity* — no clone, no `Object.freeze`, no defensive
...
mutable scratchpad — *is the cache entry from that point on*.
...
*output* becomes its *input* on the next call, so the rule applies
...
2. **Inadvertent test contamination** — a test helper reads
   ...
   *next* test in the same `describe` block reads
...
3. **Force-multiplier on finding #9** — finding #9 noted that a
   ...
   *attacker* response that has *valid* fields plus an extra
...
4. **Combined with finding #4 redirect-SSRF** — the redirected
   ...
   field names) writes the fixups *back into the cache*, so the
...
Pick (B) if you want callers to *be able* to mutate their
...
keeps and mutates that reference *before* the next cache hit
...
The third pattern requires the *second* hex character of the first
...
prefix — and stops there. 6to4 and NAT64 are *different* transition
...
3. **`fec0::/10` legacy site-local** — niche; Linux removed
   ...
   *job* is to be conservative. The defense should match the
   RFC 4193 ULA boundary (fc00::/7) *and* the deprecated-but-
   formerly-private fec0::/10 — because attackers will configure
   it where they can.

4. **IPv6 unspecified `::` as base URL** — actually *is* caught
...
*literal* and force the lookup phase (which does the embedded-IPv4
...
- `tests/client-features.test.ts`: `allowPrivateHosts: true`
  ...
  contract).
- `tests/client-features.test.ts`: 6to4 wrapping a *public*
...
The risk class (CWE-150 *Improper Neutralization of Escape,
Meta, or Control Sequences*) has a long history in CLI tools
...
- Pairs with **#10** (typo'd flags silently dropped) — an agent
  that mis-types a flag *and* passes an injection payload gets
...
Optional refinement: when stderr is *not* a TTY (e.g., piped to a
file or a CI log that does *not* render ANSI), pass through the
...
but on a different code path with a *different* exfil channel: the
...
JSON file with a valid root object passes, *all* extra fields are
forwarded.
...
3. **DoS via device file.** Agent told to use `--data-file
   ...
   is the *expected* recovery, but the work-in-flight (e.g. an
...
2. **Add a size cap.** TestRail rejects bodies beyond a few MB
   ...
   Check via `lst.size` *before* the open:
   ```ts
   if (lst.size > MAX_DATA_FILE_BYTES) {
       return { ok: false,
           error: `--data-file '${input.dataFileFlag}' is ${lst.size}` +
                  ` bytes; exceeds limit ${MAX_DATA_FILE_BYTES}.` };
   }
   ```
...
2. **Cross-project leak via run / milestone descriptions** — the
   ...
   access to *any* project in the TestRail tenant can plant
...
*umask-modified* at the syscall (`mkdir(2)` applies the
...
2. **Delete and replace the legitimate SKILL.md.** The file
   ...
   the *file* isn't world-writable. But the parent directory
   *is*, and POSIX permits unlinking any file in a writable
...
3. **Plant the symlinks that findings #5, #7, #17 already
   exploit.** This bug *enables* those bugs to be cheaply
...
1. Pass an explicit mode on the create. `0o700` for `--global`
   ...
   *does* need to read it, but no other user should write):
...
and password" are removed in the *credentials* path), AND the client
already sets an explicit `Authorization: Basic` header from
`config.email`/`config.apiKey`, so the userinfo in the URL doesn't
get forwarded as a *second* auth header on the wire. So the
...
3. **fetch error surfaces.** undici's `TypeError: fetch failed`
   ...
   classes (DNS, TLS), the cause includes the host *with*
...
1. **Operator-as-attacker-to-themselves.** An operator who
   ...
   `--email`/`--api-key` *and* userinfo in baseUrl. The library
...
3. **Force-multiplier on finding #11** (`--api-key` argv
   ...
   userinfo path is a *second* uncovered surface for the same
...
2. Add a defensive normaliser step on the stored `baseUrl` even
   ...
   guaranteed by WHATWG to *not* include userinfo, so a future
...
- `tests/client-edge-cases.test.ts`: a redirect from a
  ...
  *not* forward the userinfo to the redirect target — the
...
request *and* the response body stream — aborting the signal
errors the body read. The library wires that correctly via
`signal: controller.signal` in the `RequestInit`. The bug is
that the timer that would *fire* the abort gets cancelled the
moment headers land.
...
- **#12 (unbounded response body):** combined, the attacker
  drips bytes *and* the buffer grows unbounded. Hits the
...
streamed read so the budget enforces *time* and the cap enforces
*bytes* — two orthogonal guards.
...
- `tests/client-features.test.ts`: stub `fetch` to return a
  ...
  `TestRailApiError` *because the body wasn't fully read in
  time*, not because headers were late. Measure wall-clock —
...
- **Class signal.** The vulnerability class is well-known
  (CWE-1321), and even though *this* instance has low impact,
  the same pattern (`Record<string, …>` with prototype-typed
  default + bracket access on user input) crops up in 5 more
  places in this codebase that should be audited together:

  1. `RESOURCES[resource]` (L145) — *this finding*
  2. `HANDLERS[\`${resource}:${action}\`]` (L158) — concatenated
     key, accidentally safe: `'__proto__:foo'` isn't on
     Object.prototype, so this access returns `undefined`. But
     `resource = 'constructor', action = 'prototype'` →
     `HANDLERS['constructor:prototype']` is `undefined`. Safe
     *by accident* — change the join character and the safety
     evaporates.
...
the same time — `HANDLERS` is the dispatch table that *will*
...
The GET-LRU is a *result* cache, not an *in-flight* cache. The cache
...
1. **Rate limit exhaustion.** `checkRateLimit` (L383–405) records
   ...
   "wait 60 seconds" message. With *duplicate* IDs in the
...
3. **Force-multiplier on finding #21** (slowloris-on-body).
   If the server slow-drips, *every* stampede caller is its own
...
5. **Force-multiplier on finding #13** (POST retry idempotency).
   ...
   fail with a 5xx, *that one* retries with backoff — while
   the other N-1 succeed. The retry's exponential-backoff
   sleep happens *after* its N-1 siblings have already
...
3. **Chained with finding #9** (cache poisoning). N
   ...
   *different* outcomes (caller 1 sees 200 with shape A;
   caller 2 sees 200 with shape B during the flap). The
   last-write-wins semantics of the cache mean only one
   shape ends up cached, but each caller has *already* used
...
- **Coalesce on success *and* on rejection.** All N concurrent
...
- **Don't coalesce across `skipCache: true`** — the existing
  ...
  callers that *need* a fresh fetch; preserve that contract.
...
*only* to avoid trying to drain an interactive terminal — and the
...
size you can `lstat()` *before* opening; a stdin pipe has no size
...
ceiling for *both* paths; the stdin path also needs the
...
3. **Self-DoS via shell mis-quoting.** A human operator writes
   ...
   with a 512 MB heap cap, the call OOMs *before* the JSON parser
...
4. **Force-multiplier on finding #10** (`strict: false` typo'd
   ...
   massive stdin still gets drained *and* the real API call
   ...
   async function readStdinCapped(
       ...
       return new Promise((resolve) => {
           ...
           const timer = setTimeout(() => {
               ...
               resolve({ ok: false,
                   error: `stdin read exceeded ${timeoutMs} ms` });
           }, timeoutMs);
           process.stdin.on('data', (chunk: Buffer) => {
               ...
               if (total > maxBytes) {
                   ...
                   process.stdin.removeAllListeners();
                   resolve({ ok: false,
                       error: `stdin exceeded ${maxBytes} bytes` });
                   return;
               }
               chunks.push(chunk);
           });
...
503 is *the* canonical advertised-outage status — every well-known
...
1. **Self-DoS during real outages.** A 100-call batch under
   ...
   shutdown; here it actively works *against* graceful
   degradation.
...
4. **Time-budget thrashing with finding #21 (slowloris).** If
   ...
   *unbounded* error-body read combined with rapid retries
...
- `tests/client-features.test.ts`: `500 Internal Server Error`
  *without* `Retry-After` continues to use exponential backoff
...
true` opt-in, with this exact error message when the flag is *not*
set:
...
For comparison, the library *does* emit a runtime warning for the
...
1. **Production-as-staging drift.** A developer adds
   ...
   surfaces the leak *after* it has happened).
...
3. **Container internal HTTP.** A Kubernetes pod talks to
   ...
   captures the request URL *and the Authorization header* (a
...
- **#4 (redirect SSRF) on HTTP.** The redirect-follow path is
  ...
  http://...` — even *without* `allowInsecure: true`, because the
...
- **#11 (`--api-key` argv exposure).** Combined: the credential
  is visible via argv *and* every wire packet. A passive observer
...
2. **Suppress the warning programmatically** for legitimate
   ...
   into the trade-off. The warning fires *unless*
...
- `tests/client-features.test.ts`: `client.isInsecure()`
  returns `true` only when both `allowInsecure: true` *and*
...
2. **Decimal-looking IDs that hit scientific-notation interpretation.**
   `testrail case get 1e10` — the user *meant* an ID literally
...
3. **Whitespace-bearing pasted IDs.** An operator copies a case ID
   ...
   CLI silently trims and accepts. Looks like a feature; *is* a
...
4. **Beyond-MAX_SAFE_INTEGER precision loss.** `parseId('9007199254740993')`
   ...
   9007199254740992** — a *different* integer than the user typed.
   The CLI sends a URL for an ID the user did not specify. No
   warning, no error.

The `validateId` method in `client-core.ts:411–415` (the *programmatic*
ID guard) has the same shape — `typeof id !== 'number' ||
!Number.isInteger(id) || id <= 0` — so a programmatic caller passing
`client.getCase(0x42)` is structurally correct (0x42 *is* 66 at the JS
language level; the caller wrote a literal). The CLI's `parseId`
inherits that shape but applies it to *user-typed strings*, where the
...
- The "wrong ID returned" pattern *can* compound with agent
  trust: an agent that accepts the API response verbatim may
  attach attachments / record results against the coerced ID
  instead of the intended one. The blast radius is the *wrong
  case got attached* class.
...
export function parseId(raw: string | undefined, name: string): number {
    ...
    if (!/^[1-9]\d*$/.test(raw)) {
        throw new IdParseError(
            `${name} must be a positive decimal integer (got: ${raw})`,
        );
    }
    const n = Number(raw);
    if (!Number.isSafeInteger(n)) {
        throw new IdParseError(
            `${name} must be ≤ ${Number.MAX_SAFE_INTEGER} (got: ${raw})`,
        );
    }
    return n;
}
...
TestRail server-side caps may or may not catch it, but the *intent*
was different.
...
*later-constructed* clients that suffer most — a common pattern in
...
`auth = ''`, `activeClients.delete(this)`. None of these *should*
...
- The handlers either call `process.exit()` immediately (per
  ...
  natural exit. Either way, the error is the *last* event the
  process will see; there's no caller to surface it to.
- The cost of a silent swallow is a debugging blind spot during
  shutdown; the benefit is that **every** client that *can* be
  cleaned up *will* be. For a security-relevant cleanup loop
...
signal handler registration *opt-in* via `registerSignalHandlers:
...
- `tests/client-features.test.ts`: with three clients where the
  *middle* one's destroy throws, assert all three of the first
...
`validateEntryId` is the *only* gate between the caller's `entryId`
...
That's the *entire* validation. Anything that isn't an empty string
...
- The `?` and `#` characters **do** parse as query/fragment
  ...
  *second* `?` is just a literal char. But a `#` *does* truncate
...
1. **Prompt-injected agent passes malicious entryId.** An LLM
   ...
   resolves to a *different* TestRail operation than
...
breaks down — `entryId` *looks* validated, isn't really, and
...
- Match the *error wording* of `validateId` — start with
