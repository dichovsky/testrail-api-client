...
- [ ] 🟢 ♻️ ARCH #2: Write-handler factory — collapse 16 `cli/handlers/*-write.ts` files (66 handlers, 1503 LOC) into declarative specs over a `createWriteHandler(...)` factory — *Audited/detailed in May 2026 report*
- [ ] 🟡 ♻️ ARCH #3: Promote `ACTIONS` (`cli/metadata.ts`, 1209 LOC) to single source of truth — generate `dispatch.ts` HANDLERS (344 LOC) and `cli/index.ts` HELP text from it; new actions today require 3+ edit sites (handler + dispatch + metadata + HELP), drift caught only by tests not types
