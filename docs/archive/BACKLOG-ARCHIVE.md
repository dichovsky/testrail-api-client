...
- [x] **`user get-by-email` (GET `get_user_by_email`)** — Shipped with the "CLI user lookups (P2)" PR. Exposed as `testrail user get-by-email --email <addr>` (no positional args; extras rejected fail-fast with `IdParseError`). The `--email` flag is consumed twice by design: once by `resolveAuth()` for the HTTP Basic credential, once by the handler for the lookup query — reusing the existing `KNOWN_FLAGS` entry keeps the flag inventory minimal. Wired through `HandlerArgs.email` in `src/cli/handler-context.ts` and the `HandlerArgs` builder in `src/cli/index.ts`. The handler enforces only non-empty (post-trim); TestRail-side `EMAIL_REGEX` in `src/modules/users.ts` rejects malformed addresses with `TestRailValidationError` before any network call, so format validation isn't duplicated at the CLI boundary. Surfaces 404 (no matching user) / 401 / 403 / network as exit 1 through the standard `main()` catch path.
...
- [x] **`case add-bulk`** — Shipped with the "CLI bulk-case + run watch + attachment pagination (P2)" PR. Exposed as `testrail case add-bulk <section_id> --data '[{...},{...}]'`. New `AddCasesBulkPayloadSchema` (`z.array(AddCasePayloadSchema).min(1)`) in `src/schemas.ts` rejects empty arrays and array items that don't match `AddCasePayloadSchema` before any network call. New programmatic method `client.addCases(sectionId, payload)` wraps `POST add_cases/{section_id}` and rethrows TestRail < 7.5 errors (`Invalid uri` 400/404) as `TestRailApiError(status, 'TestRail server >= 7.5 required for add_cases bulk endpoint', <original response>)` so the version notice lands in `error.message` while the raw server body is preserved in `response` for programmatic inspection. `--dry-run` preview includes a `count` field for confirming array length against source data before a multi-hundred-case import.
...
- [x] **`result list-for-case`** — Shipped with the same PR. `result list-for-case <run_id> <case_id> [--limit N] [--offset N] [--status-id 1,5] [--defects-filter STR]` exposes `getResultsForCase(runId, caseId, options?)` → `GET get_results_for_case/{run_id}/{case_id}`. Both new commands share a `buildResultOptions()` helper in `src/cli/handlers/result.ts` so flag shape stays uniform across the two per-id reads. `result list` (the existing bulk-by-run endpoint) keeps its limit/offset-only shape unchanged — `--status-id` / `--defects-filter` are flagged as new flags wired into `src/cli/flags.ts` and the `HandlerArgs` bag, parsed by a new `parseIdList()` helper in `src/cli/ids.ts` that rejects malformed comma-lists fail-fast (instead of silently dropping bad IDs). `GetResultsOptions` gained an optional `defects_filter?: string` field; `getResults()` and `getResultsForCase()` forward it via `buildEndpoint()` (TestRail's `defects_filter` query param accepts a JIRA-key-shaped substring). `getResultsForRun()` keeps its main-branch shape and ignores `defects_filter` so the existing `result list` CLI command stays untouched.
...
- [x] 🟢 📦 CLI: `dataset get/list/add/update/delete` (P2) — Shipped
  - **Impl:** Five new actions across two handler files: `src/cli/handlers/dataset.ts` (read: `handleDatasetGet`, `handleDatasetList`) and `src/cli/handlers/dataset-write.ts` (writes: `handleDatasetAdd`, `handleDatasetUpdate`, destructive `handleDatasetDelete`). `getDataset` / `getDatasets` / `addDataset` / `updateDataset` / `deleteDataset` already existed on `TestRailClient` via `DatasetModule`; this PR added the CLI binding only. New Zod schemas `AddDatasetPayloadSchema` (`name: string`) and `UpdateDatasetPayloadSchema` (`name?: string`) in `src/schemas.ts` with passthrough so `custom_*` fields survive. `Dataset` / `AddDatasetPayload` / `UpdateDatasetPayload` interfaces migrated from `src/types.ts` to `src/schemas.ts` (matches the P1 Variables migration precedent — Zod is now the single source of truth, exported from `src/index.ts` via the schema barrel). `src/modules/datasets.ts` updated to import the payload types from `../schemas.js` instead of `../types.js`. Registered in `dispatch.ts` (5 entries) and `metadata.ts` (5 ActionSpecs, `destructive: true` on delete with `apiEndpoint` strings matching the `@testrail` JSDoc tags on `DatasetModule`). HELP text extended in `cli/index.ts` under Read actions, Write actions, the `--yes` destructive set, the `--soft`-rejected set, and the no-body writes list. Schema-name binding added to `scripts/skill-renderer.mjs` so `skill/SKILL.md` renders both payload schemas in the generated section.
  - **Rat:** P2 data-driven-testing endpoints. Datasets bind variable substitution matrices to plan-entry-triggered runs; they're the second half of the variables-and-datasets pair shipped in P1. The CLI surface stays metadata-only by upstream design — `dataset add`/`update` accept only `name`, with the actual variable-value row matrix managed via TestRail's web UI (the upstream row-CRUD API is not part of the stable public surface). `dataset delete` follows the locked-in destructive pattern: `--yes` required, `--dry-run` wins for preview-without-API, and `--soft` is rejected (TestRail's `delete_dataset` does not support soft preview — mirrors `variable delete` / `milestone delete` / `project delete`). Unit coverage in `tests/cli-write-handlers.test.ts` (happy, dry-run, body reject, path-param reject, `--soft` reject for delete, dry-run-wins-over-soft, empty-string id boundary) and `tests/cli-read-handlers.test.ts` (happy + every INVALID_IDS id boundary for both `get` and `list`); subprocess coverage in `tests/cli.test.ts` (URL/method assertion, dry-run, all id boundaries including `1e2`/`0x1`, 400/401/403/404, network error, table format, `--yes` gate, dry-run-wins-over-yes-and-soft, unknown-action rejection). Payload-schema parse/reject/`custom_*` passthrough coverage added in `tests/payload-schemas.test.ts`. Drift gates A/B/C/C2 and the metadata↔dispatch consistency suite all pass.
- [x] 🟢 📦 SKILL recipe: Data-driven runs via Variables + Datasets (P2) — Shipped
  - **Impl:** Recipe #28 in `skill/SKILL.md` (hand-written prose section between the configurations recipe and the destructive-actions reference). Tagged with three `<!-- recipe-for: -->` HTML comments (`dataset:add`, `dataset:update`, `dataset:delete`) so gate C2 binds it to the new ActionSpec entries. End-to-end walkthrough: define variables → create dataset shell → reference `${var_name}` placeholders in case steps → trigger data-driven run via plan-entry binding → per-row execution → update/delete lifecycle in parent-after-child cascade order. Includes a minimal `dataset add` payload JSONC block with a `custom_*` extra to document the `.passthrough()` contract. Snapshot tests in `tests/skill-recipes.test.ts` pin the grouping (all three tags under the same heading), the lifecycle command order (preventing reorder of the setup-then-teardown narrative), the example payload (catching accidental drift on the schema preview), and the metadata-only/web-UI caveat (catching scope-creep edits that misrepresent the CLI surface).
  - **Rat:** Closes the data-driven-testing skill surface alongside the dataset CRUD CLI. The recipe makes the variable-then-dataset workflow discoverable to agents — without it, the connection between `${var_name}` placeholders in case steps, dataset row matrices managed in the web UI, and plan-entry binding for per-row execution would only be visible to humans reading TestRail's UI docs. The destructive-cascade caveat (deleting a dataset bound to an active plan entry invalidates that entry's substitution matrix) is documented in the recipe and tested explicitly.
...
- [x] **CLI: `user add` (`POST add_user`)** — Shipped as PR #114 (`feat: CLI user add + user update`, SHA `8b90c9d`) alongside `user update`. `testrail user add` wraps the create-user endpoint with `UserAddPayloadSchema` (required: `name`, `email`, `password`; optional: `is_active`, `role_id`, `group_ids`, `mfa_required`, `language`, `email_notifications`; `.passthrough()` for future fields). Recipe #32 documents the password-UX recommendation: use `--data-file` to avoid shell-history exposure. Migrated `addUser` payload type from handwritten interface in `types.ts` to Zod-derived `UserAddPayload` from `schemas.ts`. Required password field caused 19 new test cases plus updates to `client-endpoints.test.ts` fixtures. Sub-agent's rebase against PR #113 inadvertently overwrote recipe #34 (result:add-by-test) — corrected by recovery PR #115.
- [x] **CLI: `user update` (`POST update_user/{user_id}`)** — Shipped with PR #114 (same SHA `8b90c9d`). `testrail user update <user_id>` mirrors the add path with `UserUpdatePayloadSchema` (all fields optional, PATCH semantics). Recipe #33 documents partial updates including the "deactivate without deleting history" pattern (`is_active: false`) and password-rotation via `--data-file`. Path-param validation via `parseId(ctx.args.pathParams[0], 'user_id')` rejects non-numeric, negative, and zero values before any API call.
