...
1. **Synchronous** in `validateTestRailConfig`: `isPrivateHostLiteral` rejects `localhost` and any IP literal that the shared `net.BlockList` classifies as private — loopback, RFC 1918, RFC 6598 CGNAT (`100.64.0.0/10`), link-local, `0.0.0.0/8`, IPv6 unspecified/loopback/ULA/link-local/site-local, 6to4, the well-known NAT64 prefix `64:ff9b::/96`, and the RFC 8215 local-use NAT64 prefix `64:ff9b:1::/48` (a router on such a network translates `64:ff9b:1::a00:1` to private `10.0.0.1`).
