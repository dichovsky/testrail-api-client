...
| `StepHistoryEntrySchema.id` / `user_id`                                                         | `z.string()` but wire is a **number**                                                                            | `23`, `40`      | `z.number()` / `z.number().nullish()`                        |
...
- New regression suite `tests/live-audit-regression.test.ts` (14 tests) pins every fix using **sanitized** fixtures under `tests/fixtures/live-audit/` (strings deep-scrubbed to `redacted`; numbers/booleans/null/key-presence preserved exactly — no PII committed). Confirmed RED before the fixes, GREEN after.
...
- Full gate suite green: `typecheck`, `lint`, full `vitest` (3253 passing), `codemap:check`, `mapping:check`, `agents-md:check`, `schema-conventions`.
