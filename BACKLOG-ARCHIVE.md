...
- [x] **`template list` (GET `get_templates/{project_id}`)** — Shipped with the same PR. Exposed as `testrail template list <project_id>` (one positive-integer path param; `parseId` rejects 0/-1/floats/hex/scientific notation/empty before any network call). New `src/cli/handlers/template.ts`. The three no-arg handlers (`case-field list`, `result-field list`, `status list`) inline an explicit `pathParams.length > 0` check that throws an `Error` (not `IdParseError`) so a typo like `testrail status list 5` surfaces a clear error instead of silently ignoring the `5` — a stricter contract than the precedent `case-status list` handler, intentionally locked in by unit tests.
