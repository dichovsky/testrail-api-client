...
- [x] 🟢 📦 SKILL recipe: Shared step propagation + history audit — PR #98
  - **Impl:** New hand-written recipe 28 in `skill/SKILL.md` ("Shared step propagation + history audit"), inserted between recipe 27 (Configuration hierarchy) and the `## Destructive actions` section. Tagged with four `<!-- recipe-for: shared-step:{add,update,delete,history} -->` HTML comments so gate C2 (`npm run mapping:check`) binds the recipe to the four `ActionSpec` entries already in `src/cli/metadata.ts`. The recipe walks the full lifecycle: create with `shared-step add <project_id> --data '{title, custom_steps_separated}'`, reference from cases via `custom_steps_separated[].shared_step_id`, audit prior revisions with `shared-step history <id>`, mutate with `shared-step update <id>` (propagation is immediate and server-side — every referencing case picks up the new content on next read), audit again post-mutation, then retire with `shared-step delete <id> --yes` (`--dry-run` wins; no upstream `--soft`). Notes section pins six load-bearing facts: (1) propagation is server-side and immediate, (2) delete does NOT cascade to referencing test cases (per `src/cli/handlers/shared-step-write.ts` doc comment — cases lose the reference but the case row survives), (3) `shared-step history` is the only audit mechanism (paginated via `--limit`/`--offset`), (4) TestRail does not expose a bulk reference-lookup endpoint so estimating blast radius requires walking `case list` + `jq` over `custom_steps_separated[]?.shared_step_id`, (5) `UpdateSharedStepPayloadSchema` accepts `{}` (every field optional, matching `UpdateMilestonePayloadSchema`), (6) destructive `delete` mirrors `milestone delete` / `plan delete` (`--yes` gates, no `--soft`, `--dry-run` wins). Snapshot test added in `tests/skill-recipes.test.ts` pinning: all four recipe-for tags under the same heading, the propagation-is-server-side narration, the lifecycle commands in walkthrough order, the case-side `shared_step_id` reference shape, the delete-does-not-cascade warning, the destructive-action safety pattern, the audit-before-mutate recommendation, and the no-bulk-reference-lookup gotcha. Backlog bullet removed from `BACKLOG.md`. No `src/` or `tests/cli*` changes — recipe-only PR.
  - **Rat:** P2 documentation gap flagged after PR #93 shipped `shared-step add/update/delete`. The CLI surface existed; the human-readable propagation/audit story did not. Shared steps are a fan-out primitive — a single update can mutate hundreds of cases server-side — so the recipe centers on the audit workflow (`shared-step history` before any mutation) rather than just the syntax. Verified against `src/modules/sharedSteps.ts` (method signatures + `@testrail` endpoint bindings), `src/cli/handlers/shared-step.ts` + `shared-step-write.ts` (the delete cascade clarification is lifted directly from the handler doc comment so the recipe cannot drift from code behavior), and `src/schemas.ts` (`AddSharedStepPayloadSchema` requires `title`; `UpdateSharedStepPayloadSchema` accepts `{}`). All twelve DoD steps green: typecheck, lint, codemap (no symbol changes), mapping (gate C2 picks up the four new recipe tags), build + skill (skill:check passes), format, test (1213 + 8 new snapshot cases), drift checks A/B/C/C2.
