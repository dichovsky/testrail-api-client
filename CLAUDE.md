...
| `src/upload-source.ts`                                               | A multipart upload end to end: platform path mapping, `openAsBlob`, FormData append, stream ownership, one descriptor close; `build()` is single-use                                                                                                    |
| `src/request-budget.ts`                                              | One call's wall-clock allowance: absolute time, phase clipping, promise racing, retry delays, and the single 408 it raises; injected clock                                                                                                              |
...
| `src/cli.ts`                                                         | Binary entrypoint (`bin: testrail`, `./cli` subpath): builds the real `CliRuntime` — argv, streams, stdin, signal handlers — and assigns `process.exitCode`. The only file in `src/` allowed to name `process.stdout`/`stderr`/`argv` (ESLint-enforced) |
| `src/cli/index.ts`                                                   | `runCli(runtime)`: arg parse, dispatch, auth, handler invocation. Takes everything process-shaped as a `CliRuntime` (argv, env, streams, stdin, client factory, platform, exit hooks)                                                                   |
| `src/cli/output.ts`                                                  | Owns every byte the CLI emits: `createOutput({quiet, format, stdout, stderr, stdoutIsTTY})` → `out`/`outRaw`/`outPayload`/`err`/`errRaw`; rendering, sanitization, `--quiet`, and the `--out -` payload+ack split                                       |
| `src/cli/{auth,ids,dispatch,handler-context,response-validation}.ts` | CLI infrastructure (env+flag resolution, ID parsing, handler dispatch, mismatch policy)                                                                                                                                                                 |
...
| `src/cli/diagnostics.ts`                                             | Bounded, redacted CLI error records and private diagnostic-file reservation, publication, and cleanup                                                                                                                                                   |
...
| `skill/SKILL.md`                                                     | Shipped agent skill — **body only**: when to use, auth, safety gates, output/pagination policy, and an index of the reference files. Keep it small; bulk belongs in `skill/reference/`                                                                  |
| `skill/reference/commands.md`                                        | Full command surface + CLI option reference (auto-gen; holds the `command-table` and `option-reference` sentinels)                                                                                                                                      |
| `skill/reference/recipes.md`                                         | The 133 numbered recipes, hand-written; carries every `<!-- recipe-for: -->` tag that gate C2 reads                                                                                                                                                     |
| `skill/reference/typescript-api.md`                                  | Programmatic SDK fallback, hand-written                                                                                                                                                                                                                 |
...
See **[docs/API-MAPPING.md](docs/API-MAPPING.md)** for the per-resource table of TestRail endpoint ↔ client method ↔ CLI command ↔ skill recipe. `@testrail` JSDoc tags on each module method bind methods to endpoints; `apiEndpoint` on each `ActionSpec` binds CLI commands; `<!-- recipe-for: resource:action -->` HTML comments in `skill/reference/recipes.md` bind numbered recipes. `npm run mapping:check` (run by `pretest` and CI) enforces five drift gates:
...
- **C2** — (bidirectional) every `recipe-for:` tag in `skill/reference/recipes.md` references an existing `ACTIONS` entry, **and** every `ACTIONS` entry has ≥1 `recipe-for:` binding unless `skillRecipeExempt: true` is set. The reverse direction is what enforces the CLI→skill half of the layer-coverage invariant below.
...
**Pagination contract:** the 24 endpoints carrying `pagination` metadata in `docs/testrail-endpoints.json` expose a trio. An `ActionSpec` no longer restates that contract — `src/cli/metadata/paginated-endpoints.ts` holds it keyed by `apiEndpoint`, and the CLI looks it up, so a command cannot declare a pagination shape that disagrees with the endpoint it calls. Gate E existed only to compare those copies and is gone. Two checks replace it: `_PaginatedEndpointsAreSurfaced` (compile-time, every paginated endpoint is reachable from ≥1 CLI command) and a runtime test asserting the same fact — the type assertion passes vacuously if `ActionEndpoint` ever widens to `string`, which is what `_ActionEndpointsAreLiterals` in `src/cli/metadata.ts` guards and why that alias must not be deleted. Existing `get*()` methods return the items from one response; `get*Page()` returns `Page<T>` with preserved envelope metadata; `getAll*()` follows response continuations and returns the complete bounded array. `_links.next` is authoritative, but only its validated offset and optional limit are extracted; the shared descriptor executor rebuilds a declared operation with its validated path parameters and preserved filters instead of following a supplied host/path. Legacy bare arrays are terminal. Aggregation returns no partial results and defaults to 250/page, offset 0, 100 pages, 25,000 items, 300,000 ms, and 100 MiB; hard ceilings are 250/page, 300,000 ms, and 1 GiB. `TestRailPaginationError.reason` is one of `max_pages`, `max_items`, `max_duration`, `max_bytes`, `invalid_page`, `invalid_continuation`, or `non_progress`.
...
**CLI invocation contract:** `FLAG_CATALOG` owns primitive argv syntax and projections; each `ActionSpec.flags` entry owns action applicability and requiredness, while structural capabilities admit shared pagination/body/file/write flags. `resolveActionInvocation()` rejects irrelevant or missing required inputs before stdin, auth, client construction, or handler work. `validateSuppliedFlagTypes()` is driven solely by per-occurrence argv tokens, never the last-wins `values` record, so a string flag that consumed the following flag as its value is rejected instead of silently dropping that flag's effect. Detection is structural — a consumed token leading with `--` plus at least one more character — so it does not depend on the swallowed spelling: `--filename --dry-run`, the inline-valued `--filename --dry-run=true`, the typo `--filter --dryrun`, and the repeated `--filter --dry-run --filter abc` are all rejected. The inline `--flag=--value` form consumes no token and stays valid, and is the escape hatch for a literal value that reads as a flag; `-5`, `-`, and a bare `--` are not flags and pass. `main()` owns this gate — it runs once before dispatch, so `resolveActionInvocation()` deliberately does not repeat it. Required declarations feed both CLI help and the generated skill command table. `softMode` lives only on `ActionSpec`; omission rejects real `--soft` execution, including `run close`, while `--dry-run --soft` remains side-effect free. SDK method routing stays in typed per-domain handlers rather than string-key dispatch.
...
**Retry:** `min(1000 × 2^n, 10000)` ms backoff. **GET** retries on: 5xx, 429, network errors. **POST/PUT/DELETE** retries only on 429 (rate-limited writes are rejected before execution); 5xx and network errors surface immediately to prevent duplicate writes. No retry on: 4xx, AbortError (timeout). The policy is **derived, never declared**: `deriveRetryPolicy()` (`src/retry-policy.ts`) selects it from `body.kind` + `responseKind` + `intent`, so a multipart body never retries by construction and a caller cannot spell the combination that duplicated an upload. **`Retry-After`** (RFC 7231 §7.1.3) is honored on every retryable response — 429 for all methods, and 5xx on GET (including binary downloads). The header accepts delta-seconds or HTTP-date, is capped at `MAX_RETRY_DELAY_MS`, and falls back to exponential backoff when absent, zero, in the past, or unparseable so a buggy server cannot induce a hot retry loop.
...
**Lifecycle:** Instances auto-register in module-level `activeClients Set`. `destroy()` disposes the root-owned `RequestCache` (timer plus stored/in-flight state), zeros the credential, and removes the client from the set. Process signal handlers (`exit`/`SIGINT`/`SIGTERM`) are **opt-in** via `registerProcessHandlers: true` on `TestRailConfig` (default `false`, SEC #8 — library consumers must not have their signal chain hijacked). When opted in, handlers call `destroy()` on every active instance; SIGINT/SIGTERM additionally `process.exit(130/143)`. The CLI opts in from `src/cli.ts`, deliberately outside `runCli` so a test calling it never installs handlers that cannot be removed; library callers should leave the flag off and call `destroy()` from their own shutdown hook. Once installed for a process, handlers persist for its lifetime (no safe deregistration without per-client ownership tracking).

**Output ownership:** `src/cli/output.ts` owns every byte the CLI emits, and
`src/cli.ts` is the only file in `src/` permitted to name `process.stdout`,
`process.stderr`, or `process.argv` — ESLint's `no-restricted-properties`
rejects them everywhere else. `createOutput` requires its writers instead of
defaulting to the process streams, and `HandlerContext`'s five writers
(`out`, `outRaw`, `outPayload`, `err`, `errRaw`) are non-optional, so no handler
has to cope with not being handed one. `--quiet` is decided in `createOutput`
alone; a collaborator that needs it takes a writer rather than its own flag.
One deliberate exception: `outPayload` does not gate the _payload_ on `--quiet`
(`--out -` is an explicit request for those bytes) but does gate its ack.
The TTY warning is derived from the payload type — bytes warn, text never does —
so `bdd get --out -` cannot warn about its own Gherkin. See
[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) §6.7;
`tests/cli-output-ownership.test.ts` asserts it from outside `runCli`.
...
already wrong by omission — `TESTRAIL_CONFIG_EMAIL_PATTERN`,
`MAX_PAGINATION_LIMIT`, `MAX_DATA_FILE_BYTES`, `MAX_STDIN_BYTES`,
`MAX_STDIN_UPLOAD_BYTES`, `STDIN_READ_TIMEOUT_MS` and `YAML_INDENT_SPACES` were
all exported but absent from it. A hand-maintained mirror of a source file in a
repo with five generator-backed drift gates is a liability, and one agents are
especially prone to trusting.
