...
| `src/cli.ts`                                                                | Binary entrypoint (`bin: testrail`, `./cli` subpath): builds the real `CliRuntime` — argv, streams, stdin, signal handlers — and assigns `process.exitCode`                                                |
| `src/cli/index.ts`                                                          | `runCli(runtime)`: arg parse, dispatch, auth, handler invocation. Takes everything process-shaped as a `CliRuntime`; touches `process` only for the diagnostic platform check and exit listener (ARCH #12) |
