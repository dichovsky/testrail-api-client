...
export function renderRulesBody(actions: readonly ActionLike[]): string {
    ...
    return [
        ...
        '- **CLI response drift**: entity-field mismatches warn by default;',
        '  `--strict-responses` or `TESTRAIL_STRICT_RESPONSES=1` fails on the',
        '  first mismatch. Warnings contain only method, known command, and',
        '  normalized issue codes/paths — never endpoint values or raw data.',
        ...
        '| Class                     | Thrown when                                                       |',
        '| ------------------------- | ----------------------------------------------------------------- |',
        '| `TestRailApiError`        | HTTP/network/protocol error, including malformed default lists   |',
        ...
        '',
        'A throwing response-mismatch hook propagates before downstream list/page',
        'decoders. Never interpret that hook error as proof a write did not happen.',
        '',
